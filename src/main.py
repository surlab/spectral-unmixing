...
def main(fp, i_number, j_number, channel_i, channel_j, alpha=0.01):
    ...
    fig, ax, title = plotting.plot_channels(
        channel_i, channel_j, i_number, j_number, alpha=alpha, label=fp
    )
    ...
    for i, j in zip(channel_i, channel_j):
        try:
            corrected_i.append(
                comp.correct_PMT_nonlinearity(i, detected_photons, true_photons)
            )
            corrected_j.append(
                comp.correct_PMT_nonlinearity(j, detected_photons, true_photons)
            )
