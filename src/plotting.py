...
def plot_channels(x, y, i, j, plot=True, alpha=0.01, label=""):
...
def random_plotting_code():
    ...
    ax1.scatter(im[:, :, :, i], im[:, :, :, j], alpha=0.01)
    if i < j:
        label = "Ch " + str(i + 1) + " vs Ch" + str(j + 1)
        try:
            compute_PMT_nonlinearity(
                im[:, :, :, i], im[:, :, :, j], current_data_dir, label
            )
        except Exception as E:
            pass
        ax.scatter(im[:, :, :, i], im[:, :, :, j], alpha=0.01, label=label)
    ax1.set_xlabel("Channel " + str(i + 1))
    ax1.set_ylabel("Channel " + str(j + 1))
    ...
    ax2.set_yscale("log")
    ax2.set_title(
        "Intensity spread in Channel" + str(j) + " for fixed Channel" + str(i)
    )
    ax2.legend()
    ...
    ax3.set_yscale("log")
    ax3.set_title(
        "Spread around mean in Channel" + str(j) + " for fixed Channel" + str(i)
    )
